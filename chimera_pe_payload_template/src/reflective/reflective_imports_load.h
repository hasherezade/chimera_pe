...
bool write_handle(HMODULE hLib, ULONGLONG call_via, LPSTR func_name, LPVOID modulePtr, bool is64)
{
    size_t field_size = (is64) ? sizeof(ULONGLONG) : sizeof(DWORD);
    FARPROC hProc = (FARPROC)GetProcAddress(hLib, func_name);
...
#if defined(_WIN64)
bool solve_imported_funcs_b64(LPSTR lib_name, DWORD call_via, DWORD thunk_addr, LPVOID modulePtr)
{
    ...
    do {
        ...
        if (*thunk_val == *call_via_val) {
            ...
            printf("name: %s\n", func_name);
            if (!write_handle(hLib, call_via, func_name, modulePtr, true)) {
...
bool solve_imported_funcs_b32(LPSTR lib_name, DWORD call_via, DWORD thunk_addr, LPVOID modulePtr)
{
    ...
    do {
        ...
        if (*thunk_val == *call_via_val) {
            ...
            LPSTR func_name = by_name->Name;
            printf("name: %s\n", func_name);
            if (!write_handle(hLib, call_via, func_name, modulePtr, false)) {
                //printf("Could not load the handle!\n");
                return false;
            }
        }
